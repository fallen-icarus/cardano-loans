...
wrongDatumLoanDownPayment :: EmulatorTrace ()
wrongDatumLoanDownPayment = do
  borrowerH <- activateContractWallet (knownWallet 1) endpoints
  lenderH <- activateContractWallet (knownWallet 2) endpoints
...
takeMoreAfterMeetingDownPayment :: EmulatorTrace ()
takeMoreAfterMeetingDownPayment = do
  borrowerH <- activateContractWallet (knownWallet 1) endpoints
  lenderH <- activateContractWallet (knownWallet 2) endpoints
...
tests = do
  ...
  testGroup "Accept an offer"
    ...
    , checkPredicateOptions opts "Fail if output datum has wrong loanDownPayment"
        (Test.not assertNoFailedTransactions) wrongDatumLoanDownPayment
    ...
    , checkPredicateOptions opts "Fail if collateral value deposited does not match loanDownPayment"
