...
module Test.ActiveUTxOs.AcceptOffer
  ( 
    -- * Scenarios Tested
...
-- The collateral outputs are first for both loans. The offers are from the same lender.
...
-- | Accept multiple loan offers. The loan asset is ADA and one native asset is used as collateral.
-- The lender outputs are first for both loans. The offers are from the same lender.
...
-- | Accept multiple loan offers. The loan asset is ADA and one native asset is used as collateral.
-- The lender output is first for the first loan but second for the second loan. The offers are
...
-- | Accept multiple loan offers. The loan asset is ADA and one native asset is used as collateral.
-- The lender output is second for the first loan but first for the second loan. The offers are
-- from the same lender.
...
-- | Accept a single loan offer. The loan asset is ADA and one native asset is used as collateral.
-- There is a normal output between the collateral output (which appears first) and the lender
-- output (which appears last).
...
-- | Accept a single loan offer. The loan asset is ADA and one native asset is used as collateral.
-- There is a normal output between the collateral output (which appears last) and the lender
-- output (which appears first).
...
-- | Accept multiple loan offers from different lenders.
        ...
        \(offerRef,Just offerDatum@OfferDatum{_lenderId}) ->
          ...
          in  [ TokenMint
              ...
              , TokenMint
                  ...
                  , mintRedeemer = 
                      toRedeemer $ CreateActive borrowerCred negotiationBeaconCurrencySymbol
                  , mintPolicy = toVersionedMintingPolicy activeBeaconScript
                  , mintReference = Just activeRef
                  }
              ]
...
-- | Accept multiple loan offers from the same lender but different loan assets.
...
-- | Accept a single interest-free loan offer.
...
-- | Accept a single loan offer using the CreateCloseOrUpdateAsk negotiation beacon redeemer
-- instead of BurnNegotiationBeacons. This also requires using the CloseOrUpdateAsk spending 
-- redeemer. This is not really a desired feature, but this test verifies the hypothesis. This
-- should be benign and is discincentivized by the increased cost of using CreateCloseOrUpdateAsk
-- instead of BurnNegotiationBeacons.
...
-- | Store an extraneous asset in the collateral output. This is not a desired action but borrowers
-- are incentivized not to do this since it means they are risking other unnecessary assets. This is
...
1) All loan inputs must come from the same borrower's address.
2) The borrower must approve.
3) All loan outputs must be to the borrower's loan address.
------------------------}
...
-- | When accepting a single loan offer, there is an invalid Ask UTxO from a different borrower's
-- address. The invalid UTxO is spent with CloseOrUpdateAsk redeemer.
...
-- | When accepting a single loan offer, there is an invalid Offer UTxO from a different borrower's
-- address. The invalid UTxO is spent with CloseOrUpdateOffer redeemer.
...
-- | When accepting multiple loan offers, an offer does not come from the borrower's address.
...
-- | When accepting multiple loan offers, an ask does not come from the borrower's address. The
-- other Ask UTxO is spent with the CloseOrUpdateAsk redeemer.
...
2) It should never be possible to mint unrelated active beacons.
3) It should never be possible to mint negotiation beacons.
4) It should never be possible to mint too many active beacons.
5) It should never be possible to mint beacons to a badly configured Active UTxO.
------------------------}
...
-- | When accepting a single loan offer, withdraw the second loanId instead of giving it to the
-- lender.
...
{------------------------
-- Collateral Requirement Failures

1) The required amount of collateral must be posted.
------------------------}
...
-- | Accept a single loan offer but do not leave enough collateral.
...
1) Invalid before must always be set.
2) Invalid hereafter must be set if any offers have expirations.
3) All offers must not be expired.
------------------------}
...
1) All required outputs for an offer MUST appear before any required outputs for the next offer.
   In other words, the required outputs must match the order of the inputs.
------------------------}
...
-- | When accepting multiple loan offers, another lender output appears between the required outputs
-- for the first offer.
...
-- | When accepting multiple loan offers, another collateral output appears between the required outputs
-- for the first offer.
...
1) The lender output must go to the lender's address.
2) It must contain the required deposit.
3) It must contain the LoanId for that loan.
4) It must have the proper datum.
------------------------}
...
2) The datum fields must all be properly set.
------------------------}
    ...
    emptyTxParams
      ...
      , outputs =
          [ Output
              ...
              , outputDatum = OutputDatum $ 
                  toDatum @ActiveDatum activeDatum{_rolloverFrequency = Just 1}
              , outputReferenceScript = toReferenceScript Nothing
              }
          , Output
              { outputAddress = toCardanoApiAddress lenderAddr
    ...
    emptyTxParams
      ...
      , outputs =
          [ Output
              ...
              , outputDatum = OutputDatum $ 
                  toDatum @ActiveDatum activeDatum{_lastCheckpoint = 10000000000}
              , outputReferenceScript = toReferenceScript Nothing
              }
          , Output
              { outputAddress = toCardanoApiAddress lenderAddr
...
tests :: TestTree
tests =
  testGroup "Accept Offer(s) - Basic"
    ...
    , scriptMustFailWithError "credentialFailure8" 
        "Active beacon script not executed with proper redeemer"
        credentialFailure8
      -- The exact error message depends on the order the scripts are executed.
    , scriptMustFail "credentialFailure9" credentialFailure9
