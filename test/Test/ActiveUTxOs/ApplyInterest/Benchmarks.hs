...
module Test.ActiveUTxOs.ApplyInterest.Benchmarks where
...
-- Benchmark Tests
-------------------------------------------------
-- | Apply interest to multiple Active loans. The deposit does not change for any of the loans. The
-- interest is applied once for each loan. No penalties are required.
...
-- | Apply interest to multiple Active loans. The deposit does not change for any of the loans. The
-- interest is applied three times for each loan. No penalties are required.
  ...
  let sampleRollovers acs = flip map acs $ 
        \(offerRef,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = OutputDatum $ toDatum $ createPostInterestActiveDatum 3 ad
            , outputReferenceScript = toReferenceScript Nothing
            }
    ...
    emptyTxParams
      { inputs = flip map activeUTxOs $ \(activeUtxoRef,_) ->
          Input
            { inputId = activeUtxoRef
            , inputWitness = 
                SpendWithPlutusReference loanRef InlineDatum (toRedeemer $ ApplyInterest 0 3)
            }
      , outputs = sampleRollovers activeUTxOs
      , withdrawals =
          [ Withdrawal
...
-- | Apply interest to multiple Active loans. The deposit increases by 1 ADA for all loans. The
-- interest is applied once for each loan. No penalties are required.
    ...
    emptyTxParams
      { inputs = flip map activeUTxOs $ \(activeUtxoRef,_) ->
          Input
            { inputId = activeUtxoRef
            , inputWitness = 
                SpendWithPlutusReference loanRef InlineDatum (toRedeemer $ ApplyInterest 1_000_000 3)
            }
      , outputs = sampleRollovers activeUTxOs
      , withdrawals =
          [ Withdrawal
...
-- | Apply interest to multiple Active loans. The deposit does not change for any of the loans. The
-- interest is applied once for each loan. A fixed fee penalty is required for each of them.
...
-- | Apply interest to multiple Active loans. The deposit does not change for any of the loans. The
-- interest is applied once for each loan. A percent fee penalty is required for each of them.
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all benchmark scenarios for applying interest.
tests :: [TestTree]
tests =
  [ mustSucceed "benchTest1" $ benchTest1 21
  , mustSucceed "benchTest2" $ benchTest2 21
  , mustSucceed "benchTest3" $ benchTest3 21
  , mustSucceed "benchTest4" $ benchTest4 21
  , mustSucceed "benchTest5" $ benchTest5 21
  , mustSucceed "benchTest6" $ benchTest6 21
  
  , mustExceedTxLimits "perfIncreaseTest1" $ benchTest1 22
  , mustExceedTxLimits "perfIncreaseTest2" $ benchTest2 22
  , mustExceedTxLimits "perfIncreaseTest3" $ benchTest3 22
  , mustExceedTxLimits "perfIncreaseTest4" $ benchTest4 22
  , mustExceedTxLimits "perfIncreaseTest5" $ benchTest5 22
  , mustExceedTxLimits "perfIncreaseTest6" $ benchTest6 22
  ]
