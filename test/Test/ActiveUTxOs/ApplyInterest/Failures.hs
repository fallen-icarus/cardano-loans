...
module Test.ActiveUTxOs.ApplyInterest.Failures where
...
beaconFailure5 = do
  ...
  let sampleRollovers [(_,Just ad1),(_,Just ad2)] =
        [ Output
            { outputAddress = loanAddress1
            , outputValue = utxoValue 4_000_000 $ mconcat
                [ PV2.singleton activeBeaconCurrencySymbol "Active" 1
...
beaconFailure7 = do
  ...
  void $ transact borrowerPersonalAddr [loanAddress,refScriptAddress] [borrowerPayPrivKey] $
    emptyTxParams
      ...
      , referenceInputs = [interestObserverRef,negotiationRef,activeRef,loanRef]
      , extraKeyWitnesses = [borrowerPubKey]
      , validityRange = ValidityRange
          { validityRangeLowerBound = Nothing
          , validityRangeUpperBound = Just rolloverSlot
...
-------------------------------------------------
-- | Take some collateral when applying interest to a loan.
...
-- Deposit Failures
-------------------------------------------------
-- | The deposit increase amount is negative.
...
depositFailure1 = do
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            { outputAddress = loanAddress
            , outputValue = utxoValue 3_500_000 $ mconcat
                [ PV2.singleton activeBeaconCurrencySymbol "Active" 1
                , PV2.singleton activeBeaconCurrencySymbol (_unAssetBeacon _assetBeacon) 1
                , PV2.singleton activeBeaconCurrencySymbol (_unBorrowerId _borrowerId) 1
                , PV2.singleton activeBeaconCurrencySymbol (_unLoanId _loanId) 1
...
-- | The deposit increase in the redeemer does not match the actual deposit increase. The
-- actual deposit was less.
...
-- Address Failures
-------------------------------------------------
-- | The interest output is to a different loan address.
...
-- | The interest output is to a non-loan address.
...
-- | Apply interest to multiple loans that come from different borrower addresses. The required
...
addressFailure3 = do
  ...
  let sampleRollovers loanAddress acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            { outputAddress = loanAddress
            , outputValue = utxoValue 4_000_000 $ mconcat
  ...
  void $ transact 
    ...
    emptyTxParams
      ...
      , outputs = mconcat
          [ sampleRollovers loanAddress1 activeUTxOs1 
          , sampleRollovers loanAddress2 activeUTxOs2
          ]
      , withdrawals =
          [ Withdrawal
              { withdrawalCredential = PV2.ScriptCredential $ scriptHash interestObserverScript
...
-- Address Failures
-------------------------------------------------
-- | The borrower did not approve rollover.
...
-------------------------------------------------
-- | The interest outputs are in reverse order relative to the loan inputs.
...
orderFailure1 = do
  ...
  void $ transact borrowerPersonalAddr [loanAddress,refScriptAddress] [borrowerPayPrivKey] $
    emptyTxParams
      ...
      , outputs = reverse $ sampleRollovers activeUTxOs
      , withdrawals =
          [ Withdrawal
              { withdrawalCredential = PV2.ScriptCredential $ scriptHash interestObserverScript
              , withdrawalAmount = 0
...
-- | When applying interest to multiple loans, the first loan output is wrong.
        ...
        \((offerRef,Just ad@ActiveDatum{..}),i) ->
          if odd i then
            Output
              ...
              , outputDatum = OutputDatum $ toDatum $ createPostInterestActiveDatum 0 ad
              , outputReferenceScript = toReferenceScript Nothing
              }
          else
            Output
...
-- | When applying interest to multiple loans, the second loan output is wrong.
...
-------------------------------------------------
-- Interest Application Failures
-------------------------------------------------
-- | Set the numberOfTimes in the redeemer to 0.
interestFailure1 :: MonadEmulator m => m ()
interestFailure1 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
    ...
    emptyTxParams
      { inputs = flip map activeUTxOs $ \(activeUtxoRef,_) ->
          Input
            { inputId = activeUtxoRef
            , inputWitness = 
                SpendWithPlutusReference loanRef InlineDatum (toRedeemer $ ApplyInterest 0 0)
            }
      , outputs = sampleRollovers activeUTxOs
      , withdrawals =
          [ Withdrawal
...
-- | The datum only applies the interest once even though the redeemer says twice.
interestFailure2 :: MonadEmulator m => m ()
interestFailure2 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
...
-- | When interest is applied once, a fixed fee penalty is required but it is not applied. No
-- penalty is applied.
interestFailure3 :: MonadEmulator m => m ()
interestFailure3 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
        ...
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                ...
                in OutputDatum $ toDatum @ActiveDatum $ newDatum
                    {_loanOutstanding = applyInterestNTimes False _penalty 1 _loanInterest _loanOutstanding}
            , outputReferenceScript = toReferenceScript Nothing
            }
...
-- | When interest is applied once, a percent fee penalty is required but it is not applied. No
-- penalty is applied.
interestFailure4 :: MonadEmulator m => m ()
interestFailure4 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
...
-- | When interest is applied once, a fixed fee penalty is required but it is not applied. A percent
-- fee penalty is applied instead.
interestFailure5 :: MonadEmulator m => m ()
interestFailure5 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
          ...
          Output
            ...
            , outputDatum = 
                ...
                in OutputDatum $ toDatum @ActiveDatum $ newDatum
                    {_loanOutstanding = 
                      applyInterestNTimes 
                        True (PercentFee $ Fraction (1,100)) 1 _loanInterest _loanOutstanding}
            , outputReferenceScript = toReferenceScript Nothing
            }
...
-- | When the interest is applied twice, the last epoch boundary is only increased by one epoch
-- period.
interestFailure6 :: MonadEmulator m => m ()
interestFailure6 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum@ActiveDatum{_epochDuration,_lastEpochBoundary} = createPostInterestActiveDatum 2 ad in
                OutputDatum $ toDatum $ 
                  (newDatum::ActiveDatum)
                    { _lastEpochBoundary = _lastEpochBoundary - fromJust _epochDuration 
                    }
            , outputReferenceScript = toReferenceScript Nothing
            }
...
-- | When interest is non-compounding, the interest is applied even though it shouldn't be.
-- A penalty is still applied.
interestFailure7 :: MonadEmulator m => m ()
interestFailure7 = do
  let -- Borrower Info
      borrowerWallet = Mock.knownMockWallet 1
      borrowerPersonalAddr = Mock.mockWalletAddress borrowerWallet
          ...
          Output
            ...
            , outputDatum = 
                ...
                in OutputDatum $ toDatum @ActiveDatum $ newDatum
                    {_loanOutstanding = 
                      applyInterestNTimes 
                        True _penalty 1 _loanInterest _loanOutstanding}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_borrowerId = BorrowerId ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanAsset = collateral1}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_assetBeacon = AssetBeacon ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_epochDuration = Just 999}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_lastEpochBoundary = 9999}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanTerm = 99999}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanInterest = Fraction (0,1)}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_minPayment = -1}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_collateralization = Collateralization []}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_collateralIsSwappable = True}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_claimExpiration = 0}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanExpiration = 0}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanOutstanding = Fraction (0,1)}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_loanId = LoanId ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_paymentObserverHash = ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_interestObserverHash = ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_addressUpdateObserverHash = ""}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = OutputDatumHash $ datumHash $ createPostInterestActiveDatum 1 ad
            , outputReferenceScript = toReferenceScript Nothing
            }
...
datumFailure23 = do
  ...
  -- Initialize scenario
  References{negotiationRef,loanRef,interestObserverRef} <- initializeReferenceScripts 
  mintTestTokens borrowerWallet 10_000_000 [("TestToken1",1000)]
  ...
  void $ transact borrowerPersonalAddr [loanAddress,refScriptAddress] [borrowerPayPrivKey] $
    emptyTxParams
      ...
      , referenceInputs = [negotiationRef,loanRef,interestObserverRef]
      , extraKeyWitnesses = [borrowerPubKey]
      }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_totalEpochPayments = 1000}
            , outputReferenceScript = toReferenceScript Nothing
            }
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all failure scenarios for applying interest to loans.
tests :: [TestTree]
tests =
  [ -- Beacon Failures
  ...
  , scriptMustFailWithError "orderFailure3" 
    ...
    -- Interest Application Failure Tests
  , scriptMustFailWithError "interestFailure1" 
      "Number of times not >= 1"
      interestFailure1
  , scriptMustFailWithError "interestFailure2" 
      "Not all required outputs found"
      interestFailure2
  , scriptMustFailWithError "interestFailure3" 
      "Not all required outputs found"
      interestFailure3
  , scriptMustFailWithError "interestFailure4" 
      "Not all required outputs found"
      interestFailure4
  , scriptMustFailWithError "interestFailure5" 
      "Not all required outputs found"
      interestFailure5
  , scriptMustFailWithError "interestFailure6" 
      "Not all required outputs found"
      interestFailure6
  , scriptMustFailWithError "interestFailure7" 
      "Not all required outputs found"
      interestFailure7
