...
-- | When the interest is applied twice, the last compounding is only increased by one compounding
-- period.
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum@ActiveDatum{_compoundFrequency,_lastCompounding} = createPostInterestActiveDatum 2 ad in
                OutputDatum $ toDatum $ 
                  (newDatum::ActiveDatum)
                    { _lastCompounding = _lastCompounding - fromJust _compoundFrequency 
                    }
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_compoundFrequency = Just 999}
            , outputReferenceScript = toReferenceScript Nothing
            }
  ...
  let sampleRollovers acs = flip map acs $ 
        \(_,Just ad@ActiveDatum{..}) ->
          Output
            ...
            , outputDatum = 
                let newDatum = createPostInterestActiveDatum 1 ad
                in OutputDatum $ toDatum @ActiveDatum newDatum{_lastCompounding = 9999}
            , outputReferenceScript = toReferenceScript Nothing
            }
