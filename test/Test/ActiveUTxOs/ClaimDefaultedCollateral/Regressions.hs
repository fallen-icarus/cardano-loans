...
module Test.ActiveUTxOs.ClaimDefaultedCollateral.Regressions where
...
-- | Claim the ada deposit from an expired unsecured loan.
...
-- | Terminate a loan early and claim the defaulted collateral for a loan from a borrower that has
-- missed too many payments. The borrower never made any payments on the loan.
...
regressionTest10 = do
  ...
  currentSlot >>= awaitTime . slotToPosixTime . (+2)
  claimSlot <- currentSlot
...
-- | Terminate a loan early and claim the defaulted collateral when the borrower misses too many
-- payments but the datum doesn't say it yet. The datum says some epochs have been missed but the
-- current value in the datum doesn't exceed the _maxConsecutiveMisses.
...
regressionTest11 = do
  ...
  -- Initialize scenario
  References{negotiationRef,activeRef,loanRef,paymentObserverRef,proxyRef} <- 
  ...
  currentSlot >>= awaitTime . slotToPosixTime . (+6)
  claimSlot <- currentSlot

  -- Try to claim the collateral.
  void $ transact lenderPersonalAddr [toCardanoApiAddress lenderAddr,loanAddress,refScriptAddress] [lenderPayPrivKey] $
    emptyTxParams
      { tokens = sampleKeyBurns newActiveUTxOs
