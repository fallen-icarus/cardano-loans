...
regressionTest1 = do
  ...
  let [activeDatum2] = flip map otherOfferRefs $ \(offerRef,_) ->
        createAcceptanceDatum borrowerCred1 offerRef (slotToPosixTime newStartSlot) offerDatum
...
-- | Unlock a finished loan in the same transaction where an offer is accepted.
...
failureTest2 = do
  ...
  void $ transact borrowerPersonalAddr1 [loanAddress1,refScriptAddress] [borrowerPayPrivKey1] $
    emptyTxParams
      { tokens = 
          [ TokenMint
              { mintTokens = [(_unBorrowerId borrowerBeacon1,-1)]
      ...
      , validityRange = ValidityRange
          ...
          , validityRangeUpperBound = Just $ paymentSlot + 1
          }
      }
...
-- is burned using BurnRemainderOrUnlockLost.
