...
-- Benchmark Tests
-------------------------------------------------
-- | Unlock multiple finished Active UTxOs. All loans have the same terms: the loan asset 
-- is ADA. The collateral was removed with the last payment.
...
benchTest1 number = do
  ...
  forM_ (grouped 10 activeUTxOs) $ \actives -> do
      ...
      void $ transact borrowerPersonalAddr [loanAddress,refScriptAddress] [borrowerPayPrivKey] $
        emptyTxParams
          { tokens =
              [ TokenMint
                  { mintTokens = [(_unBorrowerId borrowerBeacon,fromIntegral $ negate $ length actives)]
                  , mintRedeemer = toRedeemer BurnActiveBeacons
                  , mintPolicy = toVersionedMintingPolicy activeBeaconScript
                  , mintReference = Just activeRef
                  }
              ]
          , inputs = flip map actives $ \(activeUtxoRef,_) ->
              Input
                { inputId = activeUtxoRef
                , inputWitness = 
                    SpendWithPlutusReference loanRef InlineDatum (toRedeemer $ MakePayment 11_000_000)
                }
          , outputs = samplePayments actives
          , withdrawals =
              [ Withdrawal
