...
module Test.ActiveUTxOs.UpdateLenderAddress.Benchmarks where
...
-- | Update multiple lender payment addresses. All loans use three assets as collateral. The key NFTs
-- are in separate UTxOs.
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all benchmark scenarios for updating lender addresses.
tests :: [TestTree]
tests =
  [ mustSucceed "benchTest1" $ benchTest1 16
  , mustSucceed "benchTest2" $ benchTest2 17
  , mustSucceed "benchTest3" $ benchTest3 14
  ...
  , mustExceedTxLimits "perfIncreaseTest3" $ benchTest3 15
