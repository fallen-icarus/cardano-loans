...
-- Regression Tests
-------------------------------------------------
-- | Close a single valid Ask UTxO. The loan asset is ADA and one native asset is used as
-- collateral.
...
-- | Close multiple valid Ask UTxOs. All loans are for the same terms. One native asset is used
...
-- | Close a single valid Ask UTxO. The loan asset is ADA and three native assets are used as
-- collateral.
...
-- | Close multiple valid Ask UTxOs. All loans are for the same terms. Three native assets are used
-- as collateral for each loan.
...
regressionTest5 = do
  ...
  void $ transact borrowerPersonalAddr [loanAddress,refScriptAddress] [borrowerPayPrivKey] $
    emptyTxParams
      { tokens = flip map askUTxOs $ \(_,Just AskDatum{assetBeacon}) ->
         TokenMint
            { mintTokens = [("Ask",-1),(unAssetBeacon assetBeacon,-1)]
            , mintRedeemer = toRedeemer $ CreateCloseOrUpdateAsk borrowerCred
            , mintPolicy = toVersionedMintingPolicy negotiationBeaconScript
            , mintReference = Just negotiationRef
            }
...
-- Benchmark Tests
-------------------------------------------------
-- | Close multiple Ask UTxOs for the same loan terms. The loan asset is ada and there is one
...
-- | Close multiple Ask UTxOs for the same loan terms. The loan asset is ada and there are three
...
tests =
  testGroup "Close Ask(s)"
    ...
    , mustSucceed "benchTest2" $ benchTest2 33
    ...
    , mustExceedTxLimits "perfIncreaseTest2" $ benchTest2 34
