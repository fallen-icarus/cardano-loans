...
module Test.OfferUTxOs.UpdateOffer.Failures where
...
-- Beacon Failures
-------------------------------------------------
-- | Convert a single valid Offer UTxO to one that uses a different loan asset, but withdraw
-- the old asset beacon instead of burning it.
...
-- Credential Failures
-------------------------------------------------
-- | The lender moves the Offer UTxO to a new lender credential. The CreateCloseOrUpdateOffer
-- redeemer uses the new credential. The negotiation script is only executed as a minting policy. 
...
-- | The lender moves the Offer UTxO to a new lender credential. The CreateCloseOrUpdateOffer
-- redeemer uses the old credential. The negotiation script is only executed as a minting policy.
...
-- | The lender moves the Offer UTxO to a new lender credential. The negotiation beacon script is
-- executed as both a minting policy and a staking script. The minting execution uses the old
-- credential with CreateCloseOrUpdateOffer while the staking execution uses the new credential with
...
-- | The lender moves the Offer UTxO to a new lender credential. The negotiation beacon script is
-- executed as both a minting policy and a staking script. The minting execution uses the new
-- credential with CreateCloseOrUpdateOffer while the staking execution uses the old credential with
-- CreateCloseOrUpdateOffer.
...
-- TestTree
-------------------------------------------------
-- | A `TestTree` containing all failure scenarios for updating Offer UTxOs.
tests :: [TestTree]
tests =
  [ -- Beacon Failures
